...
public class Main9 {
    ...
    public static int approach1(int[] arr, int n, int k) {
        ...
        for (int i = 1; i < n; i++) {
            if (arr[i] - k < 0) {// if on subtracting k we got negative then continue to next iteration of the loop
                continue;
            }
            tempMin = Math.min(arr[0] + k, arr[i] - k);// Minimum element when we subtract k from whole arr
            tempMax = Math.max(arr[i - 1] + k, arr[n - 1] - k);// Maximum element when we add k to whole arr
